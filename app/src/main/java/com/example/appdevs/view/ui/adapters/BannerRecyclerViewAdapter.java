...
/**
 * @创建时间 2022/3/29 16:49
 */
public class BannerRecyclerViewAdapter extends DelegateAdapter.Adapter {
    private List<Bean> imgs;
    private Context context;
    //接收数据
    public BannerRecyclerViewAdapter(List<Bean> imgs, Context context) {
        this.imgs = imgs;
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof HomeHolder) {
            ...
            ((HomeHolder) holder).bannerItemOne.setAdapter(new BannerImageAdapter<Bean>(imgs) {
                @Override
                public void onBindView(BannerImageHolder holder, Bean data, int position, int size) {
                    //图片加载自己实现
            ...
            }).setIndicator(new CircleIndicator(context)).start();
